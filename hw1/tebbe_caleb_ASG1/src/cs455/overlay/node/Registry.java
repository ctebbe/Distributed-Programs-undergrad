...
public class Registry implements Node { //implements Runnable {
    ...
    private int counterTaskComplete;
    private ArrayList<TrafficSummary> trafficSummaryList = null;
    ...
    public Registry(int port) {
        try {
            ...
            trafficSummaryList  = new ArrayList<>();
    ...
    public synchronized void onEvent(Event event) {
        ...
        switch(event.getType()) {
            ...
            case Protocol.TASK_COMPLETE:
                if((++counterTaskComplete) == connectionMap.size()) sendPullTrafficSummaryRequests();
                break;
            case Protocol.TRAFFIC_SUMMARY:
                this.trafficSummaryList.add(((TrafficSummary) event));
                if(trafficSummaryList.size() == connectionMap.size()) 
                    StatsDisplay.calculateAndDisplayStats(trafficSummaryList);
                break;
            case Protocol.DEREGISTER:
                deregisterConnection(((Deregister) event)); 
                break;
            default:
    ...
    private void sendPullTrafficSummaryRequests() {
        try {
            Thread.sleep(50); 
            for(NodeConnection node : getNodeConnectionArray()) {
                node.sendEvent(eventFactory.buildPullTrafficSummaryEvent());
            }
        } 
        catch(IOException ioe) { display("IOE sending pull traffic requests:"+ioe.toString()); }
    ...
    public void deregisterConnection(Deregister deregisterEvent) {
        try {
            String key = deregisterEvent.getSenderKey();
            if(connectionMap.containsKey(key)) { 
                NodeConnection toRemove = connectionMap.get(key);
                connectionMap.remove(key);
                toRemove.sendEvent(eventFactory.buildRegisterResponseEvent(toRemove, Protocol.SUCCESS,
                    "Deregister request complete."));
            }
        } catch(IOException ioe) { display("Error sending regsiter request:"+ioe.toString()); }
    }
    ...
    public void run() throws IOException {
        ...
        while(input != null) {
            ...
            else if(input.contains("send-overlay-link-weights")) setupAndSendLinkWeights();
